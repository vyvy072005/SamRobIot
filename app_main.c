...
void move_backward_until_distance(int distance_cm) {
    ...
    while (traveled_distance < distance_cm) {
        ...
        /*float current_distance = 1000;
        if (current_distance >= distance_cm) {
            break;
        }*/
        // Можно добавить задержку или счетчик для контроля
        vTaskDelay(pdMS_TO_TICKS(50));
        // Обновляем пройденное расстояние
        traveled_distance += 5;/* рассчитайте по времени или сенсору */
    }
    stop_motors();
}

// Функция для поворота налево до определенного угла или времени
void turn_left_until_angle_or_time(int angle_deg) {
    int turned_angle = 0; // или используйте датчик угла
    start_turn_left(255);
    while (turned_angle < angle_deg) {
        // Можно использовать датчик угла или таймер
        // Обновляйте turned_angle по датчику
        vTaskDelay(pdMS_TO_TICKS(50));
        // Пример обновления
        turned_angle += 5;
    }
...
void distance_measurement_task(void *pvParameters) {
    const float obstacle_distance_threshold = 40.0f; // порог для препятствия
    ...
    while (1) {
        if (is_moving) {
            ...
            if (distance_cm < obstacle_distance_threshold) {
                // Обнаружено препятствие — остановка
                stop_motors();
                ...
                vTaskDelay(pdMS_TO_TICKS(300));
                 ...
                 //turn_left_until_angle_or_time(turn_angle_threshold);
                start_turn_left(255);
                vTaskDelay(pdMS_TO_TICKS(300));
                ...
                start_motors(255);
                // Отъезд назад на определенное расстояние
                //start_motors(255);
                /*start_motors_backward(255);

                vTaskDelay(pdMS_TO_TICKS(500));
                // Здесь предполагается, что у вас есть функция для движения назад
                //move_backward_until_distance(back_off_distance_cm);
                
                stop_motors();
                vTaskDelay(pdMS_TO_TICKS(300));

                // Поворот налево до определенного угла или времени
                //turn_left_until_angle_or_time(turn_angle_threshold);
                start_turn_left(255);
                vTaskDelay(pdMS_TO_TICKS(300));
                
                turning_left = true; // отметка, что мы повернули налево*/

                // Продолжение движения вперед после поворота
               // start_motors(255);
            } else {
    ...
    while (1) {
        if (is_moving) {
            ...
            if (distance_cm < 40.0f) {
                // Остановить мотор при приближении
                stop_motors();
                vTaskDelay(pdMS_TO_TICKS(500));
            } else {
                // Продолжать движение
            ...
            // Не движемся, можно сделать паузу
            vTaskDelay(pdMS_TO_TICKS(500));
        }
...
void app_main() {
    ...
    // Создайте группу событий
    s_wifi_event_group = xEventGroupCreate();

    // Настройка MQTT
    esp_mqtt_client_config_t mqtt_cfg = {
        //.broker.address.uri = "mqtt://192.168.0.101:1883",
        //.broker.address.uri = "mqtt://10.12.80.117:1883",
        .broker.address.uri = "mqtt://10.106.252.117:1883",
